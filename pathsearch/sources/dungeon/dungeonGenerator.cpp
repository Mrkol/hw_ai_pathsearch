...
void gen_drunk_dungeon(DungeonView view)
{
  ...
  const std::size_t maxSpills = 5;

  for (std::size_t iter = 0; iter < numIter; ++iter)
  {
    glm::ivec2 p = dungeon::find_walkable_tile(view);

    std::size_t numSpills = 0;
    while (numSpills < maxSpills)
    {
      if (view(p.y, p.x) == Tile::Floor)
      {
        numSpills++;
        view(p.y, p.x) = Tile::Water;
      }
      // choose random dir
      bool validDir = false;
      while (!validDir)
      {
        constexpr std::array DIRS{glm::ivec2{0, 1}, glm::ivec2{0, -1}, glm::ivec2{1, 0}, glm::ivec2{-1, 0}};
        const glm::ivec2 dir = DIRS[rndDir()];
        glm::ivec2 next = glm::min(glm::max(p + dir, glm::ivec2{1, 1}), glm::ivec2{view.extent(1), view.extent(0)} - 2);
        if (view(next.y, next.x) != Tile::Wall)
        {
          p = next;
          validDir = true;
        }
