    ...
    while (!al_event_queue_is_empty(event_queue_.get()))
    {
      ...
      switch(event.type)
      {
        ...
        case ALLEGRO_EVENT_MOUSE_BUTTON_DOWN:
          if constexpr (requires { self().mouseDown(event.mouse.button); })
          {
            if (!ImGui::GetIO().WantCaptureMouse)
              self().mouseDown(event.mouse.button);
          }
          break;

        case ALLEGRO_EVENT_MOUSE_BUTTON_UP:
          if constexpr (requires { self().mouseUp(event.mouse.button); })
          {
            if (!ImGui::GetIO().WantCaptureMouse)
              self().mouseUp(event.mouse.button);
          }
          break;
